...
speccode 编排的分支拓扑是**双层**:普通需求从 trunk 直接切 `<type>/<slug>` 开发分支(git worktree,一步直达),大需求 opt-in 走集成分支 + 父实体(子分支从集成 head 切出,终局一次 PR 上 trunk)。代码分三层(外加 `references/` 辅助层),改动前先定位属于哪一层——**确定性逻辑绝不写进命令 markdown,一律下沉到 lib**:
