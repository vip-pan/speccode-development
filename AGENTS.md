...
This file provides guidance to coding agents (Claude Code, Codex, and other CLI agents) when working with code in this repository. It is the **source of truth** for development documentation; `CLAUDE.md` is a thin Claude Code shell that merely includes this file (`@AGENTS.md`) — content MUST NOT be duplicated across the two.
...
node bin/speccode.mjs <verb> --cwd . [--flags]
```
...
5. **插件自带 hooks 层**(`hooks/`)—— Claude Code settings hook(`hooks/hooks.json` 声明、经 `${CLAUDE_PLUGIN_ROOT}` 引用,如 PreToolUse 工具输入清洗)——与 `lib/hooks.mjs` 的 config 生命周期事件是两个家族,别混淆。
...
- 全量测试(用例数量以 `tests/` 目录为准,对应关系可由 `ls tests/` 直接看到)。涉及 git 的测试用 `tests/helpers/tmprepo.mjs` 的 `makeRepo()` / `commitFile()` 建**真实临时 git 仓库**,用完清理。
- PR/等待类逻辑(`prtool` / reconcile 的 pr_open 推进)通过**依赖注入**(注入 `run` / `queryPr` / `spawn`)做单测,不依赖真实 `gh`/`glab` 或真实等待。
- CLI verb 在 `tests/cli.test.mjs` 用 `spawnSync('node', [BIN, ...])` 端到端测(写 verb 用 `input` 传 stdin),`BIN` 已用 `import.meta.url` 定位,与 cwd 无关。
