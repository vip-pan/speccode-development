...
- [ ] T2 Why 段修正:第 1 条「three-layer trunk / feature / worktree topology」→ 双层拓扑表述(trunk ↔ 开发分支直达,集成 opt-in);其余三条保留;两版同步。验证:`grep -in 'three-layer\|三层拓扑' README.md README_CN.md` 零命中(「代码三层分工」类非拓扑命中除外)。
